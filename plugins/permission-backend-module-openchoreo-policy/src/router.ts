...
export async function createRouter(
  ...
  router.post('/evaluate-with-context', express.json(), async (req, res) => {
    ...
    const entity = await catalogService.getEntityByRef(resourceRef, {
    ...
    try {
      ...
      logger.error(`Failed to fetch capabilities for ${userEntityRef}: ${err}`);
    ...
    try {
      ...
      logger.error(
        `evaluate-with-context failed for ${userEntityRef} action=${action}: ${err}`,
      );
      return res.status(200).json({ allowed: false });
    }
  });
